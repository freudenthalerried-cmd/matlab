...
| Prüfwerkzeuge | **43 Prüfer** ohne Browser, 4 Browserproben zusätzlich |
...
- **Das Repository ist öffentlich — seit dem 9. September gemessen, nicht vermutet.** Das GitHub-Werkzeug beantwortet die Frage in einem Aufruf (`visibility: public`); in der Bereitschaftsliste stand sie bis dahin als „von hier aus nicht feststellbar", und nachgesehen hatte es niemand. `.gitignore` hält die Einkaufskonditionen draußen, aber aus den veröffentlichten Verkaufspreisen und der überall dokumentierten Zielmarge sind **44 von 46 Einkaufspreisen** auf den Cent rekonstruierbar (`npm run pruefe-geheimnis`). **Empfehlung: privat stellen.**
...
- **Datenblätter der Hersteller.** Sieben Inhaltsseiten verweisen darauf, keine verlinkt eines: `baumit.at`, `schiedel.at`, `synthesa.at`, `isover.at` und `ris.bka.gv.at` sind aus der Arbeitsumgebung gesperrt — am 9. September gemessen, auf beiden verfügbaren Wegen.
