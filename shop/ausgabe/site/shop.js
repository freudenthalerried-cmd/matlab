...
const GEBIETSSTAND = Object.freeze({
  stand: '2026-08-16',
  quelle:
 ...
 * ÖNORM S 5280-2).
 */
const AUSNAHMEN_VORSORGEGEBIET = Object.freeze([
 ...
 */
function vorsorgeauskunft(bezirk) {
...
const MARGENUNTERGRENZE = 0.32;
const UST_SATZ = 0.20;

/** Kaufmännisch auf Cent runden. */
function cent(betrag) {
 ...
 */
function einkaufspreis(uvpNetto, haendlerrabatt) {
 ...
 */
function verkaufspreis(ekNetto, zielmarge, uvpNetto = Infinity) {
...
function rohmarge(ekNetto, vkNetto) {
 ...
 */
function artikelEinkauf(artikel, lieferant) {
 ...
 */
function kalkuliere(artikel, lieferant, zielmarge) {
 ...
 */
function fracht(positionen, lieferant) {
 ...
 */
function mindestbestellwertErfuellt(bestellwertNetto, lieferant) {
...
function ladeKatalog(daten, zielmarge) {
 ...
 */
function nebenkostenUntergrenze(positionen, lieferant) {
 ...
 */
function berechneBedarf(gebaeude, katalog) {
 ...
 */
function uidPruefzifferStimmt(uid) {
 ...
 */
function pruefeBestelldaten(daten = {}) {
  const fehler = [];
  const warnungen = [];
 ...
 */
function baueAuftrag(bestellnummer, kundendaten, { zahlungEingegangen = false } = {}) {
 ...
 */
function ordneEin({ wert, messdauerMonate } = {}) {
 ...
 */
function pruefeBetreiberdaten(betreiber = {}) {
...
function erzeugeImpressum(betreiber = {}) {
 ...
 */
const ZAHLUNGSBEDINGUNGEN = Object.freeze({
  zielTage: 0,
  stand: '2026-08-27',
 ...
 * gelesen.
 */
function lieferhinweise(auftrag = {}) {
...
function erzeugeBestellungen(warenkorb, auftrag) {
 ...
 */
function darfAutomatischAusgeloestWerden(warenkorb, auftrag) {
  const gruende = [];
...
const KLEINBETRAG_GRENZE_BRUTTO = 400;
const UID_EMPFAENGER_GRENZE_BRUTTO = 10000;

/**
 * Pflichtangaben nach § 11 UStG.
 * `ab` sagt, ab welcher Schwelle die Angabe verlangt wird.
 */
const RECHNUNGSMERKMALE = [
...
function erforderlicheMerkmale(bruttobetrag) {
 ...
 */
function pruefeRechnungsmerkmale(beleg = {}) {
 ...
 */
function erzeugeAngebot(warenkorb, { nummer, datum, bindefristTage = 14, kunde = {}, betreiber = {} }) {
  const zeilen = [
 ...
 */
function erzeugeAuftragsbestaetigung(
  warenkorb,
 ...
 */
function darfBestaetigtWerden(warenkorb, auftrag = {}) {
  const gruende = [];
 ...
 */
function erzeugeRechnung(warenkorb, { nummer, datum, lieferdatum, kunde = {}, betreiber = {} }) {
 ...
 */
function darfRechnungGestelltWerden(warenkorb, rechnung, auftrag = {}) {
  const gruende = [];
 ...
 * Lieferanten eine.
 */
function reihengeschaeftEinordnung(warenkorb, katalog) {
 ...
 */
function darfVorgangLaufen(vorgang) {
  const gruende = [];
 ...
 */
function ablageEintraege(vorgang, zeitpunkt) {
 ...
 */
function trockenlauf(welt = WELT_HEUTE) {
  const protokoll = [];
  let minutenMensch = 0;
  let abbruchBei = null;
 ...
 */
function aufwandProMonat(welt, bestellungenProMonat) {
  const lauf = trockenlauf(welt);
...
const VIES_ENDPUNKT = 'https://ec.europa.eu/taxation_customs/vies/rest-api/check-vat-number';
 ...
 */
function baueAnfrage(uid, { eigeneUid = null } = {}) {
 ...
 */
function werteAntwortAus(antwort) {
 ...
 */
async function pruefeUid(uid, { abruf, eigeneUid = null, versuche = 3 } = {}) {
 ...
 */
function belegzeile(auswertung, uid) {
  const teile = [
    `UID ${uid}`,
    auswertung.stand,
 ...
 */
function ergaenzeFreigabe(freigabe, auswertung) {
 ...
 * über ein neues Feld.
 */
const FELDER_DER_ABLAGE = Object.freeze({
  lfd: {
    verlangt: true,
 ...
 */
function neueAblage({ zaehler = {}, schreibe = null } = {}) {
 ...
 * § 11 zulässt.
 */
function naechsteNummer(ablage, art, jahr) {
 ...
 */
function haltefest(ablage, eintrag) {
 ...
 */
function stelleRechnungAus(ablage, rechnung, { zeitpunkt, jahr, vorgang }) {
  if (!rechnung.vollstaendig) {
    return {
      ausgestellt: false,
 ...
 */
function storniere(ablage, nummer, { grund, zeitpunkt, jahr }) {
...
function vorgangsakte(ablage, vorgang) {
 ...
 */
function pruefeNummernkreis(ablage, art, jahr) {
 ...
 */
function istStorniert(ablage, nummer) {
 ...
 */
function pruefeAblagefelder(ablage) {
 ...
 * wieder her.
 */
function journalzeile(ereignis) {
 ...
 */
function ausJournal(journal, { schreibe = null } = {}) {
 ...
 */
const SKONTO_SATZ = 0.03;
const SKONTO_FRIST_TAGE = 14;
 ...
 */
function skontoErsparnis(einkaufNetto, satz = SKONTO_SATZ) {
 ...
 */
function zahlungszielTraegt({ kundenzielTage, skontofristTage = SKONTO_FRIST_TAGE, bearbeitungstage = 2 }) {
 ...
 */
function skontoGegenGebuehr(bestellung, zahlwegId, opt = {}) {
...
function gebuehr(bruttobetrag, zahlwegId) {
 ...
 */
function wirkungAufBestellung(warenkorb, zahlwegId) {
 ...
 */
function wirkungAufMonat(lage, zahlwegId) {
...
function pruefeZahlweg(zahlwegId, lage) {
  const z = findeZahlweg(zahlwegId);
 ...
 */
function zahlwegGegenSkonto(bestellung, zahlwegId, opt = {}) {
 ...
 */
function gebuehrenanteil(zahlwegId, warenkorbNetto, frachtProBestellungNetto = 0) {
  const z = findeZahlweg(zahlwegId);
 ...
 * @param {string} zahlwegId
 */
function kaskade(lage, zahlwegId) {
 ...
 */
function noetigerUmsatz(ziel, zahlwegId) {
  const {
    zielgewinn,
    fixkosten,
    rohmarge,
 ...
 */
function mehrumsatzGegenVorkasse(ziel) {
 ...
 * drinstehen.
 */
function proBestellung(warenkorb, zahlwegId, werbeanteil) {
  const z = findeZahlweg(zahlwegId);
 ...
 */
function mindestwarenkorbFreiHaus({ rohmarge, frachtNetto, zahlwegId = 'karte-stripe' }) {
