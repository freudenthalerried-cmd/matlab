...
import {
  fingerabdruck, marke, markiere, markenbefund, MARKENMUSTER, TRENNER, WEGZUSATZ,
...
/**
 * **Der Zusatz, den der Weg anhängt — seit dem 10. September 2026.**
 *
 * An dem Tag wurde gemessen, dass die GitHub-Schnittstelle aus dieser Umgebung
 * nicht nur zu lesen, sondern auch zu beschreiben ist. Seither geht die
 * Beschreibung unverändert aus `npm run pr-text` hinaus, und niemand tippt sie
 * ab. Dabei kam heraus, dass der Übertragungsweg selbst eine Attributionszeile
 * anhängt — hinter der Marke, die sie deshalb nicht decken kann.
 *
 * *Ein Zusatz, den man kennt, ist etwas anderes als ein Zusatz, den man
 * hinnimmt.*
 */
test('Der Zusatz des Weges steht genau fest', () => {
  assert.equal(WEGZUSATZ, '\n\n---\n_Generated by [Claude Code](https://claude.ai/code)_');
  assert.equal(WEGZUSATZ.length, 58);
});

test('Eine Fassung mit dem Zusatz des Weges deckt sich trotzdem', () => {
  const echt = markiere(TEXT, QUELLE, BEFEHL);
  const b = markenbefund(echt + WEGZUSATZ);
  assert.equal(b.passt, true, b.text);
  assert.equal(b.wegzusatz, true);
  assert.equal(b.zeichen, TEXT.length, 'gedeckt ist der Text, nicht der Zusatz');
  assert.match(b.text, /Attributionszeile/);
});

test('Alles andere hinter der Marke bleibt ein Befund', () => {
  const echt = markiere(TEXT, QUELLE, BEFEHL);
  for (const dahinter of [
    '\n\nNachtrag von Hand.',
    `${WEGZUSATZ} `,
    `${WEGZUSATZ}\n\nund noch etwas`,
    '\n\n---\n_Generated by [Claude Code](https://example.invalid)_',
  ]) {
    const b = markenbefund(echt + dahinter);
    assert.equal(b.passt, false, `durchgelassen: ${JSON.stringify(dahinter)}`);
    assert.equal(b.regel, 'text-hinter-der-marke');
  }
});

test('Ohne Zusatz meldet die Fassung ihn auch nicht', () => {
  const b = markenbefund(markiere(TEXT, QUELLE, BEFEHL));
  assert.equal(b.passt, true);
  assert.equal(b.wegzusatz, false);
  assert.doesNotMatch(b.text, /Attributionszeile/);
});

test('Ein dazugeschriebener Satz fällt auch mit Zusatz auf', () => {
  // Der Fall, für den das Ganze gebaut ist, darf durch den Zusatz nicht
  // verdeckt werden.
  const echt = markiere(TEXT, QUELLE, BEFEHL);
  const dazu = echt.replace('zweite endet hier.', 'zweite endet hier. Und noch ein Satz.');
  assert.equal(markenbefund(dazu + WEGZUSATZ).regel, 'marke-passt-nicht');
});
